...
2. 笔记本电脑
   ...
   > PC 台式电脑 和 笔记本电脑 一个很重要的差别地方在于：笔记本电脑需要考虑 电源，所以 WebGPU在设计之初就要考虑 耗电量 这个关键因素。
...
2. 使用参数 `descriptor?: GPUDeviceDescriptor`
   ...
   假设我的电脑系统中只有集成显卡，就没有独立显卡，那么此时即使我添加有参数 `{ powerPreference: 'high-performance' }`，但是 GPUAdapter(显卡适配器) 依然会返回的是 集成显卡。
