...
由于 CPU 的运算速度极快，内存的数存取速度无法跟上 CPU 的速度，所以在 CPU 和 内存中间设置了数据快取区(也就是缓存区)，当计算机执行程序时，数据和地址管理部件预测可能要用到的数据和指令，并将这些数据和指令预先从内存中读出来送到缓存区。CPU 一旦需要数据时会先检查 缓存区，若有这读取(速度快)，若无则从内存中读取(速度慢)。
...
5. GPUBuffer.createBuffer() 返回一个处于 `mapped` 或 `unmapped` 状态的缓冲区。
   ...
   > 若存在映射则意味着可以被 CPU 操作。
   >
   > 请注意一旦被映射，CPU 是可以操作它了，但也意味着 GPU 不发再操作它了，直至该 GPUBuffer 实例调用 unmap() 解除映射。
...
3. GPU 内部有一个或多个 内存堆，所有正在运行的应用程序共享这些内存堆。当内存堆耗尽时 WebGPU 将无法创建新的资源。
   ...
   > 2. 堆(heap)区：动态内存分配，程序在运行时通过 new 来申请任意大小的内存，由开发者控制在适当的实际释放掉(销毁)。若程序员一直没有释放掉(销毁)则会在程序结束时被释放。
...
1. size：明确(希望)得到缓冲区的容量(字节)大小，且应该是 4 的倍数。
   ...
   > 还记得本文上面讲述 CPU 对应的 缓冲区吗？但是使用的文字是 “缓冲区所占用的存储空间是具有一定大小的”，够用即可，不要试图去 “贪心” 申请过大的容量。
